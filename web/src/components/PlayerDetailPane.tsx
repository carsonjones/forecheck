...
// import { WarChart } from '@web/components/WarChart'; // WAR not ready, section disabled
...
export function PlayerDetailPane({ player, selectedPlayerId, status, error }: Props) {
  return (
    <section className="pane entity-detail-pane">
      ...
      {player && <>
        <header className="entity-detail-header"><div><span className="eyebrow">{teamAbbreviation(player.current_team_id)} · {player.position_code} · #{player.id}</span><h1>{player.first_name} {player.last_name}</h1></div></header>
        <div className="detail-scroll entity-detail-scroll">
          {/* WAR components section disabled — not ready
